...
A lightweight and more powerful JavaScript logger for NodeJS and browsers.

`node-color-log` is a logger package for NodeJS and browsers. It provides more functions than the origin `console.log`. You can log text with colorful fonts and colorful backgrounds. Also, it has six log levels — `success`, `debug`, `info`, `warn`, `error`, and `disable`. Give you a much better experience while developing JavaScript projects.
